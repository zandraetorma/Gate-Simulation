...
public class Global {
	...
	// thread limit value will be set below 
...
// Change +2 to how many slot you want for outgate thread. 
//NOTE: threadLimit added numberOfExcel to also count the Lane thread, so if a Lane thread is done, extra slot will be available
Global.threadLimit = numberOfExcel + 2
...
static void Lane(String laneID, String laneTestData, String laneType, ArrayList<String> outLaneID, String currentReportFilePath) {
	...
	if(firstData)
	{
		...
		while (i <= rowsOfData) {
			...
			while(isElementPresent(findTestObject('Object Repository/Gate Simulation/CY Gate by Appointment/inp_container_1'), 90) == false)
			{
			...
			if (isElementPresent(findTestObject('Object Repository/Gate Simulation/CY Gate by Appointment/btn_proceedInGate'), 120))
					{
						...
						while (Global.activeThreadCount >= Global.threadLimit){
							println ("Sleeping for 10 seconds, active thread count is >= threadLimit. activeThreadCount: " + Global.activeThreadCount + "; threadLimit: "+ Global.threadLimit);
							Thread.sleep(10000)
						}
...
//		}
	...
	// Open browser if log file is found
		...
		if (laneType == "IN")
			...
			WebUI.delay(5)
			//Thread.sleep(10000);
			if (isElementPresent(findTestObject('Object Repository/Gate Simulation/CY Gate by Appointment/btn_proceed'), 10))
			{
				WebUI.click(findTestObject('Object Repository/Gate Simulation/CY Gate by Appointment/btn_proceed'))
